...
public class KVServer { // Шаблон сервера
    ...
    private void load(HttpExchange h) throws IOException { // Добавлен метод load и stop
        try {
            System.out.println("\n/load");
            if (!hasAuth(h)) {
            ...
            if ("GET".equals(h.getRequestMethod())) {
                ...
                if (!data.get(key).isEmpty() || data.get(key) != null) {
                    ...
                    System.out.println("Значение для ключа успешно получено.");
                    h.sendResponseHeaders(200, 0);
                } else {
                    System.out.println("При выгрузке произошла ошибка.");
                    h.sendResponseHeaders(400, 0);
                }
            } else {
